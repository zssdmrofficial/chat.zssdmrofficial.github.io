...
function renderPromptTools() {
  ...
  const attachLabel =
    attachCount > 0 ? `附帶資料 (${attachCount})` : '附帶資料';
  ...
  accordionHeader.addEventListener('click', (e) => {
    ...
    accordionBody.style.display = isAttachedDataAccordionOpen ? 'flex' : 'none';
    accordionHeader.querySelector('.accordion-chevron').style.transform =
      isAttachedDataAccordionOpen ? 'rotate(180deg)' : 'rotate(0deg)';
  });
  ...
  element.addEventListener('click', async (ev) => {
    ...
    if (messageBtn) {
      ...
      if (isUser) {
        ...
        if (textContentEl) {
          const originalThinkingDetails = textContentEl.querySelectorAll('.thinking-details');
