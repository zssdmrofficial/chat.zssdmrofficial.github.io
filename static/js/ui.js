...
function initCopyHandler(element) {
    ...
    element.addEventListener('click', async (ev) => {
        ...
        if (codeBtn) {
            const container = codeBtn.closest('.code-container') || codeBtn.closest('.python-analysis-indicator');
        ...
        if (regenBtn) {
            ...
            if (!Number.isFinite(regenIndex) || regenIndex < 0 || regenIndex >= history.length) return;
