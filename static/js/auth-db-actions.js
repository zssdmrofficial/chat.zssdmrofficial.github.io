async function handleDeleteAccount() {
    ...
    const confirmed = await showConfirmModal('確定要刪除帳號嗎？此動作將永久刪除您的帳號及所有對話紀錄，且無法復原。');
    if (!confirmed) return;

    const reauthenticated = await showReauthModal('為了確認您的身份，請輸入密碼以繼續刪除帳號。');
...
async function deleteConversation(convId) {
    ...
    const confirmed = await showConfirmModal('確定要刪除這個對話嗎？此動作無法復原。');
    if (!confirmed) return;
